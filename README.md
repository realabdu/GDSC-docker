...
go check http://localhost:8000/admin/ 
note that we can see all the static files served.
...
### adding Gunicorn
[Gunicorn](https://gunicorn.org/), a production-grade WSGI ( Web Server Gateway Interfac) server, that is reliable and fast.
...
you will see everything working but these is something odd. static files are not served (admin page)!!
...
###  adding nginx
we are going to need a new service so where we should add that ? 
